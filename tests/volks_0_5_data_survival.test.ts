...
export async function runDataSurvivalDisasterRecoveryTest() {
  ...
  const personsData = await dbPrimary.query<PersonRow>(`SELECT * FROM persons ORDER BY created_at ASC;`);
  const engagementsData = await dbPrimary.query<EngagementRow>(`SELECT * FROM employment_engagements ORDER BY created_at ASC;`);
