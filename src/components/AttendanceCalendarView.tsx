...
const JULY_2026_DAYS: DayDetail[] = Array.from({ length: 31 }, (_, i) => {
  const day = i + 1;
  const dateStr = `2026-07-${day.toString().padStart(2, '0')}`;
  const dayOfWeek = new Date(2026, 6, day).getDay();

  if (dayOfWeek === 0 || dayOfWeek === 6) {
    return { day, dateStr, status: 'WEEK_OFF', shift: 'General Shift', firstPunch: '--', lastPunch: '--', workingHours: '0h 0m', lateMinutes: 0, earlyMinutes: 0 };
  }
  if (day === 3) {
    return { day, dateStr, status: 'LATE', shift: '09:00 AM - 06:00 PM', firstPunch: '09:35 AM', lastPunch: '06:05 PM', workingHours: '8h 30m', lateMinutes: 35, earlyMinutes: 0, notes: 'Traffic delay' };
  }
  if (day === 7 || day === 8 || day === 9) {
    return { day, dateStr, status: 'LEAVE', shift: 'General Shift', firstPunch: '--', lastPunch: '--', workingHours: '0h 0m', lateMinutes: 0, earlyMinutes: 0, notes: 'Approved Casual Leave' };
  }
  if (day === 12) {
    return { day, dateStr, status: 'ON_DUTY', shift: 'Client Visit', firstPunch: '09:00 AM', lastPunch: '05:30 PM', workingHours: '8h 30m', lateMinutes: 0, earlyMinutes: 0, notes: 'On Duty - Client Site Bangalore' };
  }
  if (day === 15) {
    return { day, dateStr, status: 'WFH', shift: 'Remote', firstPunch: '08:55 AM', lastPunch: '06:10 PM', workingHours: '9h 15m', lateMinutes: 0, earlyMinutes: 0, notes: 'Work From Home Approved' };
  }
  if (day === 18) {
    return { day, dateStr, status: 'MISSING_PUNCH', shift: '09:00 AM - 06:00 PM', firstPunch: '09:02 AM', lastPunch: '--', workingHours: '4h 0m', lateMinutes: 2, earlyMinutes: 0, notes: 'Out punch missing' };
  }
  return { day, dateStr, status: 'PRESENT', shift: '09:00 AM - 06:00 PM', firstPunch: '08:58 AM', lastPunch: '06:02 PM', workingHours: '9h 04m', lateMinutes: 0, earlyMinutes: 0 };
});

export const AttendanceCalendarView: React.FC<{ person: any }> = ({ person }) => {
  const [selectedDay, setSelectedDay] = useState<DayDetail | null>(JULY_2026_DAYS[2]); // July 3 default
      ...
      <div style={{ flex: 1, background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '24px', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <span style={{ background: '#ECFDF5', color: '#059669', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: '700' }}>Present: 20</span>
            <span style={{ background: '#FEF3C7', color: '#D97706', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: '700' }}>Late: 1</span>
            <span style={{ background: '#EFF6FF', color: '#2563EB', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: '700' }}>Leave: 3</span>
          </div>
        </div>
